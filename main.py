...
webbrowser.open('https://t.me/connection_terminated')
intro = """ ENTER """
Anime.Fade(Center.Center(intro), Colors.green_to_black, Colorate.Vertical,
           interval=0.045, enter=True)
...
text = """ 
  ...
  └───────────────────────────┘      └───────────────────────────┘      └───────────────────────────┘
                                 ...
                                 Спасибо за использование этого клиента!                       
...
select = input(
    f'{COLOR_CODE["RED"]}☄️ {COLOR_CODE["CYAN"]} НАЖМИ ENTER ЧТОБЫ НАЧАТЬ{COLOR_CODE["CYAN"]} ')


def menu():
    choice = input(
        f'{COLOR_CODE["RED"]}☠️︎{COLOR_CODE["RED"]} Выбрать функцию ➤ {COLOR_CODE["RED"]} ')
    return choice
    ...
    if choice == '1':
        ...
        if comp_choice in ["1", "2", "3"]:
            ...
            for sender_email, sender_password in senders.items():
                for receiver in receivers:
                    ...
                    comp_body = comp_text.format(username=username.strip(),
                                                 id=id.strip(),
                                                 chat_link=chat_link.strip(),
                                                 violation_link=violation_link.strip())
                    send_email(receiver, sender_email, sender_password,
            ...
            for sender_email, sender_password in senders.items():
                for receiver in receivers:
                    ...
                    comp_body = comp_text.format(username=username.strip(),
                                                 id=id.strip())
                    send_email(receiver, sender_email, sender_password,
                               'Я утерял свой аккаунт в телеграм', comp_body)
                    print(f"Отправлено на {receiver} от {sender_email}!")
                    sent_emails += 14888
                    time.sleep(5)
        ...
        for sender_email, sender_password in senders.items():
            for receiver in receivers:
                ...
                send_email(receiver, sender_email, sender_password,
                           'Жалоба на пользователя телеграм', comp_body)
                print(f"Отправлено на {receiver} от {sender_email}!")
                sent_emails += 9999
                time.sleep(5)
        ...
        for sender_email, sender_password in senders.items():
            for receiver in receivers:
                ...
                send_email(receiver, sender_email, sender_password,
                            'Жалоба на телеграм канал', comp_body)
                print(f"Отправлено на {receiver} от {sender_email}!")
                sent_emails += 100000
                time.sleep(5)
        ...
        if bot_ch == "1":
            ...
            for sender_email, sender_password in senders.items():
                for receiver in receivers:
                    ...
                    send_email(receiver, sender_email, sender_password,
                            'Жалоба на бота телеграм', comp_body)
                    print(f"Отправлено на {receiver} от {sender_email}!")
                    sent_emails += 1
                    time.sleep(5)
